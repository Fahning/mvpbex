...
class GraficoColetaCliente extends Component
{
    public $dataGraf;
    ...
    public function mount()
    {
        $this->dataGraf = DB::table('bexsal_reports.report_076', 'cb')
            ->select(
                DB::raw("IF(con.cliente_pagador IS NOT NULL, con.cliente_pagador, 'FATURAS ANTIGAS') as pagador"),
                DB::raw('sum(cb.peso_calculo * (mv.valor_a_pagar / mv.peso_ctrcs_ctrb_coleta_entrega)) AS custo_peso'),
                DB::raw('SUM((con.cubagem_m3 * (mv.valor_a_pagar / cub.cubagem_total)))AS custo_cubagem')
            )
    ...
    public function formatData($data)
    {
        ...
        foreach ($data as $value){
            $value = (array)$value;
            array_push($dataGraf['pagador'], $value['pagador']);
    ...
    public function filtrar($filtros)
    {
        ...
        $this->dataGraf = DB::table('bexsal_reports.report_076', 'cb')
            ...
            ->when($filtros['searchCliente'], function ($query) use($filtros){
                $query->where(DB::raw("IF(con.cliente_pagador IS NOT NULL, con.cliente_pagador, 'FATURAS ANTIGAS')"), $filtros['searchCliente']);
            })
    ...
    {
        return view('livewire.components.operacional.analise-de-entregas.grafico-coleta-cliente');
    }
}
