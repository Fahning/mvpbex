    ...
    <div  class="flex mb-2">
        <div class="relative h-10 input-component">
            <x-select
                ...
                :options="[
                    ['name' => 'Veículo',  'id' => 'mf.placa_cavalo'],
                    ['name' => 'Unidade', 'id' => 'mf.und_origem'],
                    ['name' => 'Rota',   'id' => 'rota'],
                    ['name' => 'Tipo',    'id' => 'mf.RELACIONAMENTO'],
                ...
                @foreach($tablePlacas as $row)
                    <tr class="hover:bg-gray-100 border-b border-gray-200 py-2">
                        @foreach($row as $key => $value)
                            @if($key == 'Frete' || $key == 'Custo (KG)' || $key == '(R$) Mercadoria' || $key == '(R$) Frete' || $key == 'Custo (M3)' || $key == 'Valor Mercadoria')
                                <td class="px-4 py-1 w-22">{{formatReceita($value ?? 0)}}</th>
                            @elseif($key == 'placa_cavalo')
