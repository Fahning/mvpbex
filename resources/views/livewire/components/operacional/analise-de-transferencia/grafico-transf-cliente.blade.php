<div wire:init="montaChart()">
    ...
    <div wire:loading.class="hidden" class="bg-white p-3 border shadow-md rounded-md">
        <div id="chartTransfCLiente" style="height:280px;"></div>
    </div>

</div>

<script>
    window.addEventListener('renderDataTransfCLiente', ({ detail }) => {
        Highcharts.chart('chartTransfCLiente', {
            chart: {
                type: 'bar'
            },
            title: {
                text: 'Custo Transferência x Cliente'
            },
            xAxis: {
                categories: detail.newData.nome_pagador,
                title: {
                    text: null
                },
                min: 0,
