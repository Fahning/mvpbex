<div wire:init="montaChart()">
    ...
    <div wire:loading.class="hidden" class="bg-white p-3 border shadow-md rounded-md">
        <div id="chartTransfSegmento" style="height:280px;"></div>
    </div>

</div>

<script>
    window.addEventListener('renderDataTransfSegmento', ({ detail }) => {
        Highcharts.chart('chartTransfSegmento', {
            chart: {
                type: 'bar'
            },
            title: {
                text: 'Custo Transferência x Segmento'
            },
            xAxis: {
